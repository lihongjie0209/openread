...
*[zread_cli](https://www.npmjs.com/package/zread_cli) 的开源复刻版本 — 相同的功能与配置格式，完全开源。*
...
本项目是 `zread_cli` 的完全开源复刻版，保持相同的配置格式和工作流，同时带来：
...
> `zread_cli` 是 Go 程序，通过 npm 分发（`npm install -g zread_cli`）。openread 是其开源复刻版，可直接下载二进制使用。

| 功能 | `zread_cli` | **openread** |
|------|-------------|--------------|
| 安装方式 | `npm install -g zread_cli` | 直接下载二进制 |
| 源码 | ❌ 闭源 | ✅ MIT 开源 |
